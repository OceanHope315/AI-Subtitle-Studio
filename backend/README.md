...
video=<MP4 file>
analysis_mode=audio | audio_visual
```

视频文件字段名必须为 `video`，只接受 `.mp4` 扩展名和 MP4 MIME 类型，并检查文件中的
MP4 `ftyp` 标记。上传大小默认上限 1024 MB，可用 `MAX_UPLOAD_MB` 调整。成功返回 `201`
和任务 DTO；
`analysis_mode` 可选，缺省为兼容旧客户端的 `audio_visual`。音频+视觉任务新建后处于
`awaiting_roi`，此时不会提交给 AI 服务；纯音频任务不需要 ROI，上传后直接进入 `queued`
并仅提交音频识别任务，视觉来源状态为 `skipped`。
...
音频+视觉任务的完整状态顺序为
`awaiting_roi` → `queued` → `processing` → `completed` / `failed`；纯音频任务从
`queued` 开始。

任务 DTO 同时返回 `id` 和 `task_id`，以及 `analysis_mode`、`filename`、`status`、`progress`、
...
`skipped` 是明确的来源终态，不计入父任务进度；因此纯音频任务由音频来源单独决定父任务
进度与成功/失败状态。
...
纯音频任务没有视觉事件流，请求该端点会返回 `409 VISUAL_PROGRESS_NOT_APPLICABLE`，
且不会触发上游视觉事件轮询。
