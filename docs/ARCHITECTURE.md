...
1. `VideoReader.probe` 验证容器并读取 FPS、分辨率、帧数、时长和 codec。
2. `sampled_frames` 顺序 `grab` 视频，仅在采样点解码并裁剪用户的任意 x/y/w/h ROI；
...
所有保存操作都校验非负时间、`end_time > start_time`、置信度范围与坐标结构，并按
开始时间排序后原子重建 JSON/SRT。
